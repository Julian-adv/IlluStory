...
<script lang="ts">
  ...
  function dragEnter(target: HTMLElement | null) {
    // swap items in data
    if (grabbed && target != grabbed && target !== null && target.classList.contains('item')) {
  ...
  <div
    ...
    {#each data as datum, i (datum.id ? datum.id : JSON.stringify(datum))}
      <div
        ...
        class="item"
        data-index={i}
        data-id={datum.id === 0 || !!datum.id ? datum.id : JSON.stringify(datum)}
        data-grabY="0"
        on:mousedown={function (ev) {
          grab(ev.clientY, this, ev.target)
        }}
        on:touchstart={function (ev) {
          grab(ev.touches[0].clientY, this, ev.target)
        }}
