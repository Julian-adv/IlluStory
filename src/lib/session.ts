...
import { CardType, type SceneType, type StoryCard } from './interfaces'
...
export async function loadCardDialog(): Promise<StoryCard | null> {
  const selected = await open({
    filters: [{ name: '*', extensions: [presetExt, charExt, sceneExt] }]
  })
  if (typeof selected === 'string') {
