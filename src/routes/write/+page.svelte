<script lang="ts">
  ...
  async function addPrompt() {
    await tick()
    $preset.prompts = [...$preset.prompts, { id: $preset.prompts.length, role: systemRole, content: '' } ]
  }
...
<div class='grid grid-cols-[9rem,5rem,1fr] gap-0'>
  ...
  <ImageField label='Image' help='An image to show in the preset card. If empty, the image from the first scene is used.' bind:value={$preset.image} save={autoSaveFunc} />
  <StringField label='Title' placeholder='Enter title' help='Title of this preset.' bind:value={$preset.title} save={autoSaveFunc} />
  <SelectField label='API' items={apis} help='API to use.' search={false} bind:value={$preset.api} save={apiChange} />
  {#if $preset.api === Api.OpenAi}
    <StringField label='URL' placeholder='For example, https://api.openai.com/v1' bind:value={$preset.openAi.apiUrl} save={autoSaveFunc} />
    <SelectField label='Models' items={models} search={true} bind:value={$preset.openAi.model} save={autoSaveFunc} >
      <p slot='helper'>
    ...
    <NumberField label='Temperature' help='Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic.' bind:value={$preset.openAi.temperature} min={0.0} max={1.0} save={autoSaveFunc} />
    <NumberField label='Frequency penalty' help="Positive values reduce the model's tendency to repeat itself." bind:value={$preset.openAi.frequencyPenalty} min={-2.0} max={2.0} save={autoSaveFunc} />
    <NumberField label='Presence penalty' help="Positive values help the model to transition to new topics." bind:value={$preset.openAi.presencePenalty} min={-2.0} max={2.0} save={autoSaveFunc} />
    <NumberField label='Max tokens' help="The maximum number of tokens to generate in the completion." bind:value={$preset.openAi.maxTokens} min={50} max={1000} step={1} save={autoSaveFunc} />
    <NumberField label='Context size' help="Represents the model's context size. If story tokens near this, the chat history will be summarized." bind:value={$preset.openAi.contextSize} min={512} max={32768} step={1} save={autoSaveFunc} />
  {/if}
  {#if $preset.api === Api.Oobabooga}
    <StringField label='URL' placeholder='For example, http://localhost:5000/api/v1/generate' bind:value={$preset.oobabooga.apiUrl} save={autoSaveFunc} />
    <NumberField label='Temperature' help='Primary factor to control randomness of outputs. 0 = deterministic (only the most likely token is used). Higher value = more randomness.' bind:value={$preset.oobabooga.temperature} min={0.0} max={1.0} save={autoSaveFunc} />
    ...
    <NumberField label='No repeat ngram size' help="If not set to 0, specifies the length of token sets that are completely blocked from repeating at all. Higher values = blocks larger phrases, lower values = blocks words or letters from repeating. Only 0 or high values are a good idea in most cases." bind:value={$preset.oobabooga.noRepeatNgramSize} min={0} max={20} step={1} save={autoSaveFunc} />
    <NumberField label='Min length' help="Minimum generation length in tokens." bind:value={$preset.oobabooga.minLength} min={0} max={2000} step={1} save={autoSaveFunc} />
    <StringField label='Seed' bind:value={$preset.oobabooga.seed} save={autoSaveFunc} />
    <CheckField label='Do sample' help="Whether or not to use sampling ; use greedy decoding otherwise." bind:value={$preset.oobabooga.doSample} save={autoSaveFunc} />
    <NumberField label='Penalty alpha' help="Contrastive Search is enabled by setting this to greater than zero and unchecking “do_sample”. It should be used with a low value of top_k, for instance, top_k = 4." bind:value={$preset.oobabooga.penaltyAlpha} min={0.0} max={5.0} save={autoSaveFunc} />
    <NumberField label='Number of beams' help="For Beam Search, along with length_penalty and early_stopping." bind:value={$preset.oobabooga.numBeams} min={1} max={20} step={1} save={autoSaveFunc} />
    <NumberField label='Length penalty' help="length_penalty > 0.0 promotes longer sequences, while length_penalty < 0.0 encourages shorter sequences." bind:value={$preset.oobabooga.lengthPenalty} min={-5.0} max={5.0} step={0.1} save={autoSaveFunc} />
    <CheckField label='Early stopping' help="Controls the stopping condition for beam-based methods, like beam-search." bind:value={$preset.oobabooga.earlyStopping} save={autoSaveFunc} />
    <NumberField label='Truncation length' help="The leftmost tokens are removed if the prompt exceeds this length. Most models require this to be at most 2048." bind:value={$preset.oobabooga.truncationLength} min={0} max={16384} step={256} save={autoSaveFunc} />
  ...
  {/if}
  <TextField label='Visualize prompt' help="The prompt to use for visualize the conversation." bind:value={$preset.visualizePrompt} save={autoSaveFunc} />
  <TextField label='Summarize prompt' help="The prompt to use for summarizing the conversation." bind:value={$preset.summarizePrompt} save={autoSaveFunc} />
</div>

<h1 class='text-lg font-semibold mb-1 mt-3'>Prompts</h1>
<DragDropList bind:data={$preset.prompts} onChange={autoSaveFunc} removesItems let:datum={prompt} let:i >
