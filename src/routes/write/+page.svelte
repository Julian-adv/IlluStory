  ...
  <div class="grid grid-cols-[9rem,6rem,1fr] gap-0">
    ...
    {#if $preset.api === Api.Infermatic}
      ...
      <SelectField
        ...
        items={models}
        search={true}
        bind:value={$preset.infermatic.body.model}
        save={autoSaveFunc} />
      <NumberField
        label="Max tokens"
        help="The maximum number of tokens to generate in the completion."
        bind:value={$preset.infermatic.body.max_tokens}
        defaultValue={defaultPreset.infermatic.body.max_tokens}
        min={48}
        max={1024}
        step={8}
        save={autoSaveFunc} />
      <NumberField
        label="Temperature"
        help="Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic."
        bind:value={$preset.infermatic.body.temperature}
        defaultValue={defaultPreset.infermatic.body.temperature}
        min={0.01}
        max={1.99}
        save={autoSaveFunc} />
      <NumberField
        label="Top p"
        help="If not set to 1, select tokens with probabilities adding up to less than this number. Higher value = higher range of possible random results."
        bind:value={$preset.infermatic.body.top_p}
        defaultValue={defaultPreset.infermatic.body.top_p}
        min={0.0}
        max={1.0}
        save={autoSaveFunc} />
      <NumberField
        label="Seed"
        help="RNG seed to use for sampling. If not specified, the global RNG will be used."
        bind:value={$preset.infermatic.body.seed}
        defaultValue={defaultPreset.infermatic.body.seed}
        min={-1}
        max={999999}
        step={1}
        save={autoSaveFunc} />
      <NumberField
        label="Min p"
        help="Tokens with probability smaller than `(min_p) * (probability of the most likely token)` are discarded. This is the same as top_a but without squaring the probability."
        bind:value={$preset.infermatic.body.min_p}
        defaultValue={defaultPreset.infermatic.body.min_p}
        min={0.0}
        max={1.0}
        save={autoSaveFunc} />
      <NumberField
        label="Repetition penalty"
        help="Exponential penalty factor for repeating prior tokens. 1 means no penalty, higher value = less repetition, lower value = more repetition."
        bind:value={$preset.infermatic.body.repetition_penalty}
        defaultValue={defaultPreset.infermatic.body.repetition_penalty}
        min={1.0}
        max={2.0}
        save={autoSaveFunc} />
      <NumberField
        label="Frequency penalty"
        help="Repetition penalty that scales based on how many times the token has appeared in the context. Be careful with this; there's no limit to how much a token can be penalized."
        bind:value={$preset.infermatic.body.frequency_penalty}
        defaultValue={defaultPreset.infermatic.body.frequency_penalty}
        min={-2.0}
        max={2.0}
        save={autoSaveFunc} />
      <NumberField
        label="Presence penalty"
        help="Similar to repetition_penalty, but with an additive offset on the raw token scores instead of a multiplicative factor. It may generate better results. 0 means no penalty, higher value = less repetition, lower value = more repetition."
        bind:value={$preset.infermatic.body.presence_penalty}
        defaultValue={defaultPreset.infermatic.body.presence_penalty}
        min={-2.0}
        max={2.0}
        save={autoSaveFunc} />
      <NumberField
        label="Top k"
        help="Similar to top_p, but select instead only the top_k most likely tokens. Higher value = higher range of possible random results."
        bind:value={$preset.infermatic.body.top_k}
        defaultValue={defaultPreset.infermatic.body.top_k}
        min={0}
        max={100}
        step={1}
        save={autoSaveFunc} />
      <NumberField
        ...
        bind:value={$preset.infermatic.body.min_tokens}
        defaultValue={defaultPreset.infermatic.body.min_tokens}
        min={48}
        max={1024}
        step={8}
        save={autoSaveFunc} />
      <NumberField
        label="Length penalty"
        help="Used by beam search only. `length_penalty > 0.0` promotes longer sequences, while `length_penalty < 0.0` encourages shorter sequences."
        bind:value={$preset.infermatic.body.length_penalty}
        defaultValue={defaultPreset.infermatic.body.length_penalty}
        min={-5.0}
        max={5.0}
        step={0.1}
        save={autoSaveFunc} />
      <CheckField
        label="Early stopping"
        help="Used by beam search only. When checked, the generation stops as soon as there are 'num_beams' complete candidates; otherwise, a heuristic is applied and the generation stops when is it very unlikely to find better candidate."
        bind:value={$preset.infermatic.body.early_stopping}
        defaultValue={defaultPreset.infermatic.body.early_stopping}
        save={autoSaveFunc} />
      <CheckField
        label="Skip special tokens"
        help="When decoding the generated tokens, skip special tokens from being converted to their text representation. Otherwise, BOS appears as `<s>`, EOS as `</s>`, etc."
        bind:value={$preset.infermatic.body.skip_special_tokens}
        defaultValue={defaultPreset.infermatic.body.skip_special_tokens}
        save={autoSaveFunc} />
    {/if}
    {#if $preset.api === Api.KoboldAi || $preset.api === Api.Infermatic}
      ...
      <TextField
        label="System prefix"
        help="String to prefix system role message."
        bind:value={$preset.systemPrefix}
        save={autoSaveFunc} />
      <TextField
        label="System postfix"
        help="String to postfix system role message."
        bind:value={$preset.systemPostfix}
        save={autoSaveFunc} />
      <TextField
        label="User prefix"
        help="String to prefix user role message."
        bind:value={$preset.userPrefix}
        save={autoSaveFunc} />
      <TextField
        label="User postfix"
        help="String to postfix user role message."
        bind:value={$preset.userPostfix}
        save={autoSaveFunc} />
      <TextField
        label="Assistant prefix"
        help="String to prefix assistant role message."
        bind:value={$preset.assistantPrefix}
        save={autoSaveFunc} />
      <TextField
        label="Assistant postfix"
        help="String to postfix assistant role message."
        bind:value={$preset.assistantPostfix}
        save={autoSaveFunc} />
    {/if}
    <TextField
