    ...
    state = await nextState(state, {
      ...
      'modified': async () => {
        ...
        if ($settings.translateInput) {
          ...
          return { state: 'translated', value: translated, translated: translated, entered: trimmed }
        } else {
  ...
  async function translate() {
    state = await nextState(state, {
      ...
      'modified': async () => {
        ...
        return { state: 'translated', value: translated, translated: translated, entered: state.value }
      },
      'translated': () => {
...
<div>
  <Markdown bind:value={state.value} readOnly={false} {placeholder} {onEnter} {onArrowUp} {onArrowDown} {onModify}/>
</div>
<div>
