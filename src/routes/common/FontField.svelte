...
<div class='col-span-2 p-1' on:mouseenter={showHelper} on:mouseleave={hideHelper}>
  ...
  <div class="flex items-center gap-2">
    <Button color='alternative' size='sm'>Choose font<Icon name="chevron-down-solid" class="w-3 h-3 ml-2 text-white dark:text-white" /></Button>
    <Dropdown bind:open={dropdownOpen} class="overflow-y-auto px-3 pb-3 text-sm h-60">
      ...
      {#each filteredFonts as font}
        ...
        <li class="rounded p-2 hover:bg-gray-100 dark:hover:bg-gray-600" on:click={selectFont(font)}>
          {font}
        </li>
      {/each}
    </Dropdown>
