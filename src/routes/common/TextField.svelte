...
<div class="w-36 flex p-1 justify-end items-center">
  <Label for={id} class="text-base text-right">{label}</Label>
</div>
<!-- svelte-ignore a11y-no-static-element-interactions -->
<div class="col-span-2 p-1 flex items-center" on:mouseenter={showHelper} on:mouseleave={hideHelper}>
  <FlexibleTextarea
    {id}
    {placeholder}
    bind:value
    on:blur={save}
    unWrappedClass="px-2 py-1 text-base prompt" />
</div>
<div></div>
<!-- svelte-ignore a11y-no-static-element-interactions -->
